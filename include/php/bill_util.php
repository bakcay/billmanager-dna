...
function Debug($str, $level = 'DEBUG') {
	global $log_file;
	$date = date("M j H:i:s");
	$pid = getmypid();
	$module = __MODULE__;
	
	$color = "\033[1;33m"; // Default yellow
	...
	fwrite($log_file, "$date [$pid] $module {$color}$level $str\033[0m\n");
}
...
function LocalQuery($function, $param, $auth = NULL) {
	...
	Debug("Executing: " . preg_replace('/password=([^\s]+)/', 'password=***', $cmd), 'INFO');
	...
	Debug("mgrctl response: " . $out_str);

	$xml = @simplexml_load_string($out_str);
	if ($xml === false) {
		throw new ISPErrorException(
			"INVALID_XML",
			"Invalid XML response from mgrctl",
			"Response: " . $out_str
		);
	}
...
class ISPErrorException extends Exception
{
	...
	function __construct($message, $object = "", $value = "", $param = array()) {
		...
		// Log error
		Error(sprintf(
			"Error: %s. Object: %s. Value: %s. Params: %s",
			$message,
			$object,
			$value,
			json_encode($param)
